...
# Installs Jupyter + NLP environment on a minimal Linux VDS.
# Includes: torch, transformers, pandas, matplotlib, spacy, nltk.
# Usage:
...
install_python_packages() {
  ...
  if [[ "$INSTALL_NOTEBOOK_DEPS" == "1" ]]; then
    log "Installing NLP dependencies..."
    python -m pip install --upgrade \
      transformers datasets accelerate evaluate \
      pandas numpy scipy matplotlib seaborn \
      scikit-learn tqdm ipywidgets \
      spacy nltk regex sentencepiece \
      tokenizers pydantic

    log "Downloading spaCy model en_core_web_sm..."
    python -m spacy download en_core_web_sm 2>/dev/null || log "spaCy model download optional"
...
verify_install() {
  # shellcheck disable=SC1090
  source "$VENV_DIR/bin/activate"
  log "Verifying Jupyter/Torch/NLP packages..."
  python - <<'PY'
import importlib

mods = ["jupyterlab", "notebook", "ipykernel", "torch", "transformers", "datasets", "pandas", "sklearn", "spacy", "nltk", "matplotlib"]
for m in mods:
    try:
...
try:
    ...
    print("spaCy model loaded:", nlp.meta.get("name", "en_core_web_sm"))
except Exception as e:
    print("spaCy model check skipped:", e)
PY
}
